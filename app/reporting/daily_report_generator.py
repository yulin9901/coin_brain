    ...
    def _generate_market_summary(self, target_date: str) -> Dict[str, Any]:
        ...
        try:
            with self.db_manager.get_connection(dictionary=True) as (connection, cursor):
                ...
                cursor.execute(query, (target_date, tuple(self.trading_pairs)))
    ...
    def _generate_top_performers(self, target_date: str) -> Dict[str, Any]:
        ...
        try:
            with self.db_manager.get_connection(dictionary=True) as (connection, cursor):
                ...
                SELECT 
                    symbol,
                    price_change_percent_24h as change_24h,
                    volume_24h,
                    current_price
                FROM market_data 
                WHERE DATE(timestamp) = %s 
                AND symbol IN %s
                AND price_change_percent_24h IS NOT NULL
                ORDER BY price_change_percent_24h DESC
                """
    ...
    def _generate_news_summary(self, target_date: str) -> Dict[str, Any]:
        ...
        try:
            with self.db_manager.get_connection(dictionary=True) as (connection, cursor):
                ...
                ORDER BY sentiment_score DESC, published_at DESC
                LIMIT 10
                """
